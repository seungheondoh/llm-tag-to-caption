...
def train(model, logger, train_dataloader,eval_dataloader, tokenizer, optimizer, lr_scheduler, gradient_accumulation_steps, train_config, loggerfsdp_config=None, local_rank=None, rank=None):
    """
    ...
    for epoch in range(train_config.num_epochs):
        with MemoryTrace() as memtrace:  # track the memory usage
            ...
            for step, batch in enumerate(tqdm(train_dataloader,colour="blue", desc=f"Training Epoch{epoch}")):
                ...
                lr_scheduler.step()
                logger.update_training_log(step, loss.detach().float(), lr_scheduler.get_lr()[0])
